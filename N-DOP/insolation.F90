...
subroutine insolation(t, latitude, iswr)
    ...
    implicit none
    ! arguments
    real(kind = rkind)  :: t, latitude, iswr
    ! constants
    real(kind = rkind), parameter :: pi = 3.141592653589793d0
    real(kind = rkind), parameter :: solar  = 1360.d0
    real(kind = rkind), parameter :: albedo = 0.6d0
    ! work vars
    real(kind = rkind)  :: dayrad, delta, latrad, sun, dayhrs, cosz, dayfrac
