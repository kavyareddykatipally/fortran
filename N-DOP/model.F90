...
subroutine metos3dbgc(n, nz, m, nbc, ndc, dt, q, t, y, u, bc, dc, ndiag, diag)
    ...
    real(kind = rkind)  :: dt, q(nz, n), t, y(nz, n), u(m), bc(nbc), dc(nz, ndc), diag(nz, ndiag)
...
subroutine NDOPmodel(n, nz, m, dt, q, t, yN, yDOP, u, phi, sigmaice, z, dz)
    ...
    ! input variables
    integer :: n, nz, m
    real(kind = rkind)  :: dt, q(nz, n), t, yN(nz), yDOP(nz), u(m), phi, sigmaice, z(nz), dz(nz)

    ! constants
    integer, parameter :: jeuphotic = 2                         ! number of euphotic layers
    real(kind = rkind), parameter  :: sigmaPAR  = 0.4d0                     ! photosynthetically available radiation (PAR)
    real(kind = rkind), parameter  :: yPstar    = 0.002777777777777778d0    ! [mmolP/m^3]

    ! work vars
    integer :: j, k
    real(kind = rkind)  :: yNj, yDOPj, ISWR, Ij, Ijprime, Ikm1, Ik
    real(kind = rkind)  :: kw, muP, KN, KI, sigmaDOP, lambdaDOPprime, b
    real(kind = rkind)  :: fP, E
    real(kind = rkind)  :: sigmaDOPbar, dtbio
